...
export default function MemberCard({ member, index, onClick }) {
  return (
    <motion.div
      ...
      custom={index % 3} // Synchronized cleanly with your 3-column configuration
    ...
    >
      <div className="relative overflow-hidden aspect-square bg-slate-100">
        ...
        <div className="absolute top-3 left-3">
          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-white/90 backdrop-blur-sm ${DEPT_TEXT[member.department] || "text-slate-500"}`}>
            <span className={` rounded-full ${DEPT_ACCENT[member.department] || "bg-slate-400"}`} />
            {member.department}
          </span>
        </div>
      ...
      <div className="p-4 sm:p-5">
        ...
        <p className="text-[11px] text-slate-400 font-semibold mb-4 leading-tight">{member.role}</p>
        
        {/* Role Section */}
        {member.description && (
          <div className="mb-4">
            <div className="flex items-center gap-2 mb-1.5">
              <span className="text-[9px] font-black uppercase tracking-wider text-slate-700">Role</span>
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[9px] font-black uppercase tracking-wider text-slate-700">Skills</span>
            <div className="h-[1px] flex-1 bg-slate-100" />
          </div>
