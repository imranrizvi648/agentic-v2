...
export default function CareersHero() {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => { clearTimeout(resizeTimer); resizeTimer = setTimeout(doResize, 150); });
    ro.observe(canvas);
    doResize();
    ...
    const getSprite = (type, ci, radius) => {
      ...
      const [r,g,b]=PALETTE[ci]; const s=document.createElement("canvas"); const sc=s.getContext("2d");
      if (type==="orb") {
        const rad=radius*3,pad=20,sz=Math.ceil(rad*2+pad*2); s.width=s.height=sz; const cx=sz/2,cy=sz/2;
      ...
      if (type==="star") {
        ...
        sc.closePath(); sc.fill(); const sp={canvas:s,cx,cy}; cache.set(key,sp); return sp;
      }
    ...
    class P {
      ...
      update(){
        ...
        this.x+=this.vx;this.y+=this.vy;this.vx*=0.97;this.vy*=0.97;this.life-=this.decay;
      ...
      draw(){
        const a=this.life,sp=this.sprite; ctx.globalAlpha=a;
    ...
    const drawLinks=()=>{
      const grid={};
      for(let i=0;i<particles.length;i++){const cx=Math.floor(particles[i].x/CELL),cy=Math.floor(particles[i].y/CELL),k=`${cx},${cy}`;(grid[k]=grid[k]||[]).push(i);}
      const seen=new Set();
      for(let i=0;i<particles.length;i++){
        ...
        for(let nx=cx-1;nx<=cx+1;nx++)for(let ny=cy-1;ny<=cy+1;ny++){
          ...
          for(const j of nb){if(j<=i)continue;const pk=i*1000+j;if(seen.has(pk))continue;seen.add(pk);
            const pj=particles[j],dx=pi.x-pj.x,dy=pi.y-pj.y,d2=dx*dx+dy*dy;
            if(d2<CELL*CELL){const d=Math.sqrt(d2),alpha=(1-d/CELL)*0.18*Math.min(pi.life,pj.life);
              ctx.globalAlpha=alpha;const[r1,g1,b1]=PALETTE[pi.ci],[r2,g2,b2]=PALETTE[pj.ci];
    ...
    const FRAME_MS=1000/40; let lastTs=0;
    const render=(ts)=>{
      if(!running)return; raf=requestAnimationFrame(render);
      if(ts-lastTs<FRAME_MS)return; lastTs=ts-((ts-lastTs)%FRAME_MS); tick++;
      ...
      for(let i=0;i<lines.length;i++){
        ...
        ctx.beginPath();ctx.moveTo(l.sx,l.sy);ctx.bezierCurveTo(l.cp1x,l.cp1y+drift,l.cp2x,l.cp2y+drift*0.5,l.ex,l.ey);
    ...
    return()=>{stop();io.disconnect();ro.disconnect();clearTimeout(resizeTimer);document.removeEventListener("visibilitychange",onVis);canvas.removeEventListener("mousemove",onMouse);canvas.removeEventListener("mouseleave",onLeave);cache.clear();};
  },[]);

  return (
    <section aria-label="Careers page header"
      className="relative w-full h-[55vh] sm:h-[65vh] md:h-[72vh] bg-[#1a194d] overflow-hidden flex items-center font-sans">
      <canvas ref={canvasRef} aria-hidden="true" className="absolute inset-0 w-full h-full block"/>
      <div aria-hidden="true" className="absolute inset-0 pointer-events-none bg-gradient-to-r from-[rgba(18,16,60,0.82)] via-[rgba(18,16,60,0.55)] to-transparent"/>
      ...
      <div className="relative z-10 max-w-7xl w-full mx-auto px-6 sm:px-10 lg:px-16 py-16 sm:py-20 lg:py-24">
        {/* Eyebrow */}
        <div className="inline-flex items-center gap-2 px-3 py-1.5 border border-[#625eff]/30 rounded-full mb-6">
          <span className="w-1.5 h-1.5 rounded-full bg-[#625eff] animate-pulse"/>
          <span className="text-[#625eff] text-xs font-bold uppercase tracking-[0.2em]">We're Hiring</span>
        </div>
