          ...
          {painPoints.map((item) => (
            <div key={item.problem} className="rounded-xl p-5 bg-slate-50 border border-slate-100 hover:border-indigo-200 transition-colors">
              <p className="text-xs font-semibold text-rose-500 mb-3 flex gap-2 items-start">
                <span className="flex-shrink-0 w-4 h-4 rounded-full bg-rose-100 flex items-center justify-center text-rose-500 font-black text-[10px]">✗</span>
                <span>{item.problem}</span>
              </p>
              <p className="text-xs text-slate-600 flex gap-2 items-start">
                <span className="flex-shrink-0 w-4 h-4 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 font-black text-[10px]">→</span>
