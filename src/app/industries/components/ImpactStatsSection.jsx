...
export default function ImpactStatsSection() {
  ...
  return (
    <section 
      className="relative py-14 sm:py-20 md:py-24 bg-cover bg-center bg-no-repeat overflow-hidden select-none"
      style={{
        // Premium corporate architectural blueprint image
    ...
    >
      {/* 
        ...
        This merges the architectural background image with a luxurious deep tech look.
      */}
        ...
        */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-[#625eff]/20 border border-[#625eff]/25 rounded-none overflow-hidden">
          {stats.map((s, idx) => (
            ...
            >
              ...
              <div className="text-4xl sm:text-5xl font-mono font-black text-white mb-3 tracking-tight group-hover:text-[#625eff] transition-colors duration-200 tabular-nums">
                {s.val}
              </div>

              {/* Label using monospaced clear uppercase structure */}
              <div className="text-[10px] font-mono font-bold text-slate-300 uppercase tracking-widest leading-relaxed mb-2">
                {s.label}
              </div>

              {/* Subtitle with proper contrast */}
              <div className="text-[10px] font-mono text-slate-400/80 leading-normal">
                {s.sub}
              </div>
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-white/5 pt-6">
          <p className="text-[10px] font-mono text-slate-400 uppercase tracking-wider text-center sm:text-left">
            // OPERATIONAL BASELINE: ALL DEPLOYMENTS ARE SECURED AND MONITORED IN REAL-TIME
          </p>
