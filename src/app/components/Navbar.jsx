...
const servicesMegaMenu = [
  {
    ...
    subServices: [
      { title: "Agentic AI for the Office of Finance", href: "/services/agentic-ai-finance", desc: "AI agents that own AP, AR, GL and month-end close end to end, natively inside your ERP." },
      { title: "Enterprise ERP & CRM-Native AI Integration", href: "/services/erp-crm-ai-integration", desc: "Connect AI agents directly to SAP B1, Oracle Fusion, NetSuite and Salesforce via official APIs." },
      { title: "Intelligent Document Processing", href: "/services/intelligent-document-processing", desc: "Turn unstructured invoices and vendor documents into posted, reconciled ERP transactions." },
    ],
    spotlight: { title: "Generative AI in Business Operations: C-Suite AI Adoption Strategies", cta: "Get the white paper", img: "/ai-and-data-innovation.png" },
  },
  {
    ...
    subServices: [
      { title: "Business Process Automation Beyond Finance", href: "/services/business-process-automation", desc: "Extend agentic AI into procurement, HR, supply chain and IT operations." },
      { title: "Custom AI Agent Development", href: "/services/custom-ai-agent-development", desc: "Bespoke multi-agent systems built with LangGraph, FastAPI and enterprise-grade LLMs." },
      { title: "Conversational AI & Enterprise Copilots", href: "/services/conversational-ai-enterprise-copilots", desc: "Let your teams query ERP data in plain language permission-aware, grounded in live records." },
    ],
    spotlight: { title: "Modernizing Legacy Apps for the AI Era", cta: "Watch Now", img: "/automation-excellence.png" },
  },
  {
    ...
    subServices: [
      { title: "Agentic AI Strategy & Readiness Consulting", href: "/services/agentic-ai-strategy-consulting", desc: "A prioritised roadmap built on what is actually buildable in your environment." },
      { title: "Data Intelligence & Decision Analytics", href: "/services/data-intelligence-decision-analytics", desc: "Predictive analytics, anomaly detection and decision-support agents on Microsoft Fabric and Oracle." },
    ],
    spotlight: { title: "How Businesses Can Move Past the AI Pilot Phase", cta: "Watch Now", img: "/strategic.png" },
  },
  {
    ...
    subServices: [
      { title: "AI Governance, Security & MLOps", href: "/services/ai-governance-security-mlops", desc: "Audit-ready agents monitored, controlled and accountable from day one." },
    ],
    spotlight: { title: "The Ultimate Guide to Boost Application Performance", cta: "Read More", img: "/ai-governance-security.png" },
  },
];
...
const industriesMegaMenu = [
  {
    ...
    spotlight: { title: "The Future of Smart Data Management in Capital Markets", cta: "Download Playbook", img: "/finance-and-banking.jpg" },
  },
  {
    ...
    spotlight: { title: "Transforming Modern Healthcare with Agentic Frameworks", cta: "Read Industry Report", img: "/healthcare.png" },
  },
  {
    ...
    spotlight: { title: "Maximizing Digital Assets and Property Returns via Advanced AI", cta: "Watch Webinar", img: "/Real-estate.png" },
  },
  {
    ...
    spotlight: { title: "Scaling Modern Enterprise Stores to New Peaks", cta: "View Case Study", img: "/ecommerce.png" },
  },
  {
    ...
    spotlight: { title: "Reimagining Digital Education Ecosystems globally", cta: "Get White Paper", img: "/education.png" },
  },
];
...
const productsMegaMenu = [
  {
    ...
    spotlight: { title: "How Leadsense hits 2-minute reply times for enterprise sales.", cta: "Explore Leadsense", img: "/leadSensedashboard.webp" },
  },
  {
    ...
    spotlight: { title: "Resolve 85% of inbound inquiries without touching a human queue.", cta: "Explore Liaison", img: "/liaisondashboard.webp" },
  },
  {
    ...
    spotlight: { title: "38% average connection accept rate across our pilot customers.", cta: "Explore Linkedinagent", img: "/linkdineadashboard.webp" },
  },
  {
    ...
    spotlight: { title: "100% FBR regulatory compliance with zero manual filing overhead.", cta: "Explore Invoicesense", img: "/invoicesensedashboard.webp" },
  },
];
...
export default function Navbar() {
  ...
  const isLightPage = pathname === "/proposal" || pathname.startsWith("/blog") || pathname.startsWith("/case-studies") || pathname === "/our-product/leadsense" || pathname === "/our-product/liaison" || pathname === "/our-product/linkedinagent" || pathname === "/our-product/invoicesense";
      ...
      >
        ...
        <div className="w-full flex items-center justify-between relative">
          ...
          <Link href="/" className="flex items-center gap-3 cursor-pointer group z-50">
            ...
            <span className={`font-bold text-base md:text-lg tracking-wide transition-colors duration-300 ${useDarkTheme ? "text-[#1a194d]" : "text-white"}`}>
              AgenticSense
            </span>
          </Link>
              ...
              >
                <Link href={item.href}>
                  ...
                  >
                    {activeTab === item.name && (
                      <>
                        <div className={`absolute top-1.5 left-1.5 w-2 h-2 border-t-[1.5px] border-l-[1.5px] rounded-tl-[2px] ${useDarkTheme ? "border-[#1a194d]" : "border-white"}`} />
                        <div className={`absolute bottom-1.5 right-1.5 w-2 h-2 border-b-[1.5px] border-r-[1.5px] rounded-br-[2px] ${useDarkTheme ? "border-[#1a194d]" : "border-white"}`} />
                      </>
                    )}
                    ...
                    {hoveredTab === item.name && (
                      <motion.div
                        ...
                        className={`absolute top-full left-0 right-0 mx-auto mt-4 w-[90vw] max-w-[1200px] min-h-[410px] flex rounded overflow-hidden border shadow-2xl z-50 backdrop-blur-2xl
                          ${useDarkTheme ? "bg-white border-slate-200 text-slate-800" : "bg-[#0b0a14]/95 border-white/10 text-white"}`}
                      >
                        <div className={`w-[25%] flex flex-col p-5 gap-1 border-r ${useDarkTheme ? "bg-slate-50/50 border-slate-100" : "bg-white/[0.02] border-white/5"}`}>
                          {servicesMegaMenu.map((menuItem) => (
                            <button key={menuItem.id} onMouseEnter={() => setActiveServicesTab(menuItem.id)} onClick={() => setActiveServicesTab(menuItem.id)}
                              className={`flex items-center justify-between px-4 py-3 text-left font-semibold text-[13.5px] rounded-lg transition-all duration-200
                                ${activeServicesTab === menuItem.id ? useDarkTheme ? "bg-[#625eff]/10 text-[#625eff]" : "bg-white/10 text-white" : "hover:bg-slate-100/50 dark:hover:bg-white/5 opacity-80"}`}>
                              <span>{menuItem.label}</span>
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={`transition-transform duration-200 ${activeServicesTab === menuItem.id ? "translate-x-1" : ""}`}><path d="M9 5l7 7-7 7" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            </button>
                          ))}
                        </div>
                          ...
                          {currentServicesData.subServices.map((sub, idx) => (
                            <Link key={idx} href={sub.href} onClick={() => setHoveredTab(null)} className="group/item flex flex-col gap-1.5 cursor-pointer">
                              <h4 className="font-bold text-[14.5px] flex items-center gap-2 group-hover/item:text-[#625eff] transition-colors">
                                {sub.title}
                                {sub.isNew && <span className="bg-blue-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider">New</span>}
                              </h4>
                              <p className={`text-[12.5px] leading-relaxed ${useDarkTheme ? "text-slate-500" : "text-slate-400"}`}>{sub.desc}</p>
                            </Link>
                          ))}
                        </div>
                        <div className={`w-[25%] p-8 flex flex-col justify-between border-l ${useDarkTheme ? "bg-slate-50/40 border-slate-100" : "bg-white/[0.01] border-white/5"}`}>
                          <div className="flex flex-col justify-start">
                            ...
                            <div className="mt-3 relative rounded-lg overflow-hidden border border-white/5 aspect-[16/10] bg-gradient-to-br from-[#625eff]/20 to-purple-600/20 flex items-center justify-center shadow-sm max-h-[140px]">
                              {currentServicesData.spotlight.img ? <img src={currentServicesData.spotlight.img} alt="Spotlight" className="w-full h-full object-cover" /> : <span className="text-xs text-slate-500">Banner Asset</span>}
                            </div>
                            <h5 className="mt-4 font-bold text-[13px] leading-snug tracking-normal">{currentServicesData.spotlight.title}</h5>
                          </div>
                      ...
                      >
                        <div className={`w-[25%] flex flex-col p-5 gap-1 border-r ${useDarkTheme ? "bg-slate-50/50 border-slate-100" : "bg-white/[0.02] border-white/5"}`}>
                          {productsMegaMenu.map((menuItem) => (
                            <button key={menuItem.id} onMouseEnter={() => setActiveProductsTab(menuItem.id)} onClick={() => setActiveProductsTab(menuItem.id)}
                              className={`flex items-center justify-between px-4 py-3 text-left font-semibold text-[13.5px] rounded-lg transition-all duration-200
                                ${activeProductsTab === menuItem.id ? useDarkTheme ? "bg-[#625eff]/10 text-[#625eff]" : "bg-white/10 text-white" : "hover:bg-slate-100/50 dark:hover:bg-white/5 opacity-80"}`}>
                              <span>{menuItem.label}</span>
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={`transition-transform duration-200 ${activeProductsTab === menuItem.id ? "translate-x-1" : ""}`}><path d="M9 5l7 7-7 7" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            </button>
                          ))}
                        </div>
                        <div className="w-[50%] p-8 grid grid-cols-2 gap-x-8 gap-y-6 items-start content-start">
                        ...
                        <div className={`w-[25%] p-8 flex flex-col justify-between border-l ${useDarkTheme ? "bg-slate-50/40 border-slate-100" : "bg-white/[0.01] border-white/5"}`}>
                          <div className="flex flex-col justify-start">
                            <span className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Spotlight</span>
                            <div className="mt-3 relative rounded-lg overflow-hidden border border-white/5 aspect-[16/10] bg-gradient-to-br from-[#625eff]/20 to-purple-600/20 flex items-center justify-center shadow-sm max-h-[140px]">
                              {currentProductsData.spotlight.img ? <img src={currentProductsData.spotlight.img} alt="Spotlight" className="w-full h-full object-cover" /> : <span className="text-xs text-slate-500">Banner Asset</span>}
                            </div>
                      ...
                      >
                        <div className={`w-[25%] flex flex-col p-5 gap-1 border-r ${useDarkTheme ? "bg-slate-50/50 border-slate-100" : "bg-white/[0.02] border-white/5"}`}>
                          {industriesMegaMenu.map((menuItem) => (
                            <button key={menuItem.id} onMouseEnter={() => setActiveIndustriesTab(menuItem.id)} onClick={() => setActiveIndustriesTab(menuItem.id)}
                              className={`flex items-center justify-between px-4 py-3 text-left font-semibold text-[13.5px] rounded-lg transition-all duration-200
                                ${activeIndustriesTab === menuItem.id ? useDarkTheme ? "bg-[#625eff]/10 text-[#625eff]" : "bg-white/10 text-white" : "hover:bg-slate-100/50 dark:hover:bg-white/5 opacity-80"}`}>
                              <span>{menuItem.label}</span>
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={`transition-transform duration-200 ${activeIndustriesTab === menuItem.id ? "translate-x-1" : ""}`}><path d="M9 5l7 7-7 7" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            </button>
                          ))}
                        </div>
                        <div className="w-[50%] p-8 grid grid-cols-2 gap-x-8 gap-y-6 items-start content-start">
                        ...
                        <div className={`w-[25%] p-8 flex flex-col justify-between border-l ${useDarkTheme ? "bg-slate-50/40 border-slate-100" : "bg-white/[0.01] border-white/5"}`}>
                          <div className="flex flex-col justify-start">
                            <span className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Spotlight</span>
                            <div className="mt-3 relative rounded-lg overflow-hidden border border-white/5 aspect-[16/10] bg-gradient-to-br from-[#625eff]/20 to-purple-600/20 flex items-center justify-center shadow-sm max-h-[140px]">
                              {currentIndustriesData.spotlight.img ? <img src={currentIndustriesData.spotlight.img} alt="Spotlight" className="w-full h-full object-cover" /> : <span className="text-xs text-slate-500">Banner Asset</span>}
                            </div>
                ...
                {item.name === "Our Insights" && item.dropdown && (
                  <AnimatePresence>
                    {hoveredTab === item.name && (
                      ...
                      >
                        <div className="p-2">
                          <Link href="/blog" onClick={() => setHoveredTab(null)}
                            className={`group/ins flex items-start gap-3 p-2 rounded-lg transition-all duration-200 ${useDarkTheme ? "hover:bg-[#625eff]/[0.06]" : "hover:bg-white/[0.06]"}`}>
                           
                            <div className="flex-1 min-w-0">
                              <h4 className={`font-bold text-[14.5px] mb-1 group-hover/ins:text-[#625eff] transition-colors ${useDarkTheme ? "text-[#1a194d]" : "text-white"}`}>Blog</h4>
                             
                            </div>
                          
                          </Link>
                          <div className={`mx-4 my-1 h-px ${useDarkTheme ? "bg-slate-100" : "bg-white/[0.06]"}`} />
                          <Link href="/case-studies" onClick={() => setHoveredTab(null)}
                            className={`group/ins flex items-start gap-2 p-2 rounded-lg transition-all duration-200 ${useDarkTheme ? "hover:bg-[#625eff]/[0.06]" : "hover:bg-white/[0.06]"}`}>
                           
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                <h4 className={`font-bold text-[14.5px] group-hover/ins:text-[#625eff] transition-colors ${useDarkTheme ? "text-[#1a194d]" : "text-white"}`}>Case Studies</h4>
          ...
          {/* ── Mobile Hamburger ──────────────────────────────────────────── */}
          <button
            className={`lg:hidden p-2 z-50 focus:outline-none transition-colors duration-300 ${useDarkTheme ? "text-[#1a194d]" : "text-white"}`}
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              className={`w-full lg:hidden overflow-hidden mt-3 pt-3 border-t ${useDarkTheme ? "border-black/[0.06]" : "border-white/[0.10]"}`}
            >
                    ...
                    <div className="flex items-center justify-between w-full rounded-lg">
                      <Link
                        ...
                        className={`flex-1 px-4 py-2.5 font-medium text-[15px] transition-colors
                          ${useDarkTheme ? "text-slate-700 hover:bg-[#1a194d]/[0.06] hover:text-[#1a194d]" : "text-white/85 hover:bg-white/[0.08] hover:text-white"}`}
                      >
                        {item.name}
                      </Link>
                      {item.dropdown && (
                      ...
                      <AnimatePresence>
                        {mobileServicesOpen && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                            ...
                            {servicesMegaMenu.flatMap((cat) => cat.subServices.map((sub) => (
                              <Link key={sub.href + sub.title} href={sub.href} onClick={() => setIsMobileMenuOpen(false)}
                                className={`py-1.5 text-[14px] font-semibold transition-colors ${useDarkTheme ? "text-slate-600 hover:text-[#625eff]" : "text-white/75 hover:text-white"}`}>
                                {sub.title}
                              </Link>
                            )))}
                          </motion.div>
                        ...
                        {mobileProductsOpen && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                            ...
                            {productsMegaMenu.map((prod) => (
                              <Link key={prod.id} href={prod.href} onClick={() => setIsMobileMenuOpen(false)}
                                className={`py-1.5 text-[14px] font-semibold transition-colors ${useDarkTheme ? "text-slate-600 hover:text-[#625eff]" : "text-white/75 hover:text-white"}`}>
                                {prod.label}
                              </Link>
                            ))}
                        ...
                        {mobileIndustriesOpen && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                            ...
                            {industriesMegaMenu.map((ind) => (
                              <Link key={ind.id} href={ind.href} onClick={() => setIsMobileMenuOpen(false)}
                                className={`py-1.5 text-[14px] font-semibold transition-colors ${useDarkTheme ? "text-slate-600 hover:text-[#625eff]" : "text-white/75 hover:text-white"}`}>
                                {ind.label}
                              </Link>
                            ))}
