...
const cases = [
  { 
    id: 1, 
    title: "Multi-entity distribution group: 80% of invoices posted straight-through inside SAP Business One", 
    tags: ["Financial Services", "AP Automation"], 
    logo: "Enterprise Finance", 
  ...
  { 
    id: 2, 
    title: "Investment firm finance copilot: Oracle Fusion data answered in seconds, not analyst-hours", 
    tags: ["Investment Management", "Finance Copilot"], 
    ...
    <section className="pt-16 sm:pt-20 md:pt-24 bg-white overflow-hidden font-sans">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 sm:mb-12 gap-4 sm:gap-6">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-brand-primary tracking-tight leading-tight">
        ...
        {/* Carousel Container */}
        <div ref={scrollRef} className="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-8 sm:pb-12 no-scrollbar" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
          {cases.map((item) => (
            <article 
              key={item.id}
            ...
            >
              <div className="w-full sm:w-[340px] md:w-[380px] flex-shrink-0 flex flex-col h-full relative z-10">
                ...
                <div className="mb-8 sm:mb-10">
                  <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-brand-primary leading-snug mb-4 sm:mb-6">
                    {item.title}
                  </h3>
                  <div className="flex flex-wrap gap-2 sm:gap-3">
                    {item.tags.map((tag, idx) => (
                      <span key={idx} className={`px-3 sm:px-4 py-1 rounded-full text-[10px] sm:text-[11px] font-bold border uppercase tracking-wider ${getTagStyle(tag)}`}>
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="mb-auto">
                  <span className="text-2xl sm:text-3xl font-black text-[#625eff] tracking-tighter opacity-80 group-hover:opacity-100 transition-opacity uppercase">
                    {item.logo}
                  </span>
                </div>
                ...
                <div className="border-t border-slate-100 pt-5 sm:pt-6 h-[130px] sm:h-[140px] flex-shrink-0">
                  <div className="flex gap-4 sm:gap-5 items-start">
                    ...
                    {/* Circle Avatar Default User Placeholder System */}
                    <div className="w-12 h-12 sm:w-14 sm:h-14 flex-shrink-0 rounded-full bg-slate-200 border border-slate-300 flex items-end justify-center overflow-hidden relative">
                      {/* Avatar SVG Graphic Layout */}
                      <svg 
                        className="w-full h-full text-slate-400 translate-y-[4px]" 
                        fill="currentColor" 
                        viewBox="0 0 24 24"
                      >
                    ...
                    <div className="flex-1">
                      <p className="text-[13px] sm:text-[14px] text-slate-600 leading-relaxed mb-3 line-clamp-3 italic">
                        "{item.quote}"
                      </p>
                      <div className="flex flex-col">
                        <span className="text-[14px] sm:text-[15px] font-bold text-[#1a194d]">
                          {item.author}
                        </span>
                        <span className="text-[12px] sm:text-[13px] text-slate-400 mt-0.5">
                          {item.role}
                        </span>
                      </div>
                    </div>
              ...
              <div className="hidden md:block absolute top-10 right-10 w-[240px] h-[160px] opacity-0 translate-x-8 transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:opacity-100 group-hover:translate-x-0 z-20">
                <div className="relative w-full h-full rounded-none overflow-hidden shadow-2xl border border-slate-200">
                  ...
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 w-max">
                    <button className="bg-white px-4 py-2 rounded-none text-[11px] font-bold text-[#1a194d] flex items-center gap-1.5 shadow-md hover:text-[#625eff] transition-colors uppercase border border-slate-200">
                      View case study
