...
export default function AgenticSenseParticleText() {
  ...
  useEffect(() => {
    ...
    // ── 1. تبدیل کریں: ویاؤ پورٹ کی چوڑائی کے مطابق فونٹ سائز بڑھا دیا ──
    const getFontSize = () => Math.max(36, window.innerWidth * 0.145);
    ...
    const buildMask = () => {
      ...
      maskCtx.letterSpacing = `${Math.round(-fs * 0.05)}px`;
  ...
  return (
    // ── 2. تبدیل کریں: text-center شامل کیا تا کہ کنٹینر میں متن ہمیشہ درمیان میں رہے ──
    <div ref={wrapRef} className="relative w-full select-none text-center block overflow-hidden" style={{ lineHeight: 1 }}>
      <div
        aria-hidden="true"
        style={{
          // ── 3. تبدیل کریں: فونٹ سائز 13vw سے بڑھا کر 14.5vw کیا اور ٹریکنگ (letter-spacing) تھوڑی ایڈجسٹ کی ──
          fontSize: "14.5vw", fontWeight: 900,
          letterSpacing: "-0.05em", lineHeight: 1.14,
